...
/**
 * Reads a user's SSH public keys from the provider they linked and registers them against their proxy account.
 *
 * <p>Runs at link time, on the token the OAuth flow has just obtained. Nothing re-reads them afterwards: a key removed
 * upstream stops working on the push regardless of what fogwall holds, since the forward authenticates against the
 * provider with the client's own agent. A user who wants their set re-read unlinks and links again.
 *
 ...
 * when the account genuinely has no keys. Collapsing the two would report a provider outage or a revoked token to the
 * user as "this account has no keys", which sends them looking in the wrong place.
 */
@Slf4j
