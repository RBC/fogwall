...
(BYO-token, egress assumption) and [docs/USER_GUIDE.md](USER_GUIDE.md#submitting-prsmrs-through-fogwall) for how a
developer points `gh`/`glab` at it.

GitHub and GitLab use different wire formats under the hood — GitHub's is GraphQL with an opaque node ID that must be
resolved to `owner/repo`; GitLab's is REST with `owner/repo` directly in the URL, so no resolution step exists for it
(`node-id-cache-ttl` below is a GitHub-only setting, inert for GitLab). See
[docs/internals/SCM_API_PROXY.md](internals/SCM_API_PROXY.md) for the per-provider wire-format detail. Both dialects
share the same opt-in config shape — a global TTL/rule block, and a per-provider `enabled` flag that actually mounts the
path:
...
proposals:
  ...
  # Provider-level access rules: is a provider allowed to serve READ and/or MUTATE SCM API traffic at all. Fail-closed
  # like every other fogwall rule/permission check — a provider with proposals.enabled: true but no matching allow rule
  # here still denies everything. Deny always takes precedence over allow. This is intentionally its own rule shape,
  # not the path-based `rules:` block above — GitHub's GraphQL requests have no owner/repo in the URL to match
  # against, only in variables the request body carries (see the internals doc's "why not UrlRuleRegistry" note).
  allow:
    - provider: github
      operation: BOTH # READ, MUTATE, or BOTH (default)
    - provider: gitlab
      operation: BOTH
  deny: [] # same shape; evaluated first
...
**Reads** (GraphQL `query` traffic — an ordinary `gh issue list`, for example) are not gated by `PROPOSE` at all; only
the provider-level `allow`/`deny` rules above apply, keeping the default read cost near pass-through (no allowlist, no
node-ID resolution, no extra round-trip).
...
| `proposals.allow[]`                            | list    | `[]`    | Provider-level allow entries: `provider`, `operation` (`READ`/`MUTATE`/`BOTH`, default `BOTH`). |
| `proposals.deny[]`                             | list    | `[]`    | Same shape as `allow[]`; takes precedence when both match.                                      |
