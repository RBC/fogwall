 ...
 * (SIN, SSN, NINO, etc.) to scan pushed diffs, commit messages and SCM API prose against. WARN-only on the push path;
 * blocking on the SCM API path, which has no reviewer to show a warning to - see {@link ContentPatternConfig}.
 *
