...
/** Integration tests for {@link JdbcNodeIdCache} backed by an H2 in-memory database. */
class JdbcNodeIdCacheTest {

    JdbcNodeIdCache cache;

    @BeforeEach
    void setUp() {
        DataSource ds = DataSourceFactory.h2InMemory("node-id-cache-test-" + UUID.randomUUID());
        new JdbcPushStore(ds).initialize();
        cache = new JdbcNodeIdCache(ds, Duration.ofMinutes(5));
    }
    ...
    void store_expiredEntry_returnsEmpty() {
        ...
        new JdbcPushStore(ds).initialize();
        JdbcNodeIdCache expiredCache = new JdbcNodeIdCache(ds, Duration.ZERO);
