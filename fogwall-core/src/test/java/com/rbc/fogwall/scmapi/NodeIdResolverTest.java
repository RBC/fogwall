...
/**
 * Exercises {@link NodeIdResolver} against a local stub GraphQL endpoint, covering the response-parsing and
...
class NodeIdResolverTest {
    ...
    private final InMemoryNodeIdCache cache = new InMemoryNodeIdCache();
    ...
    void cacheHit_neverCallsUpstream() {
        ...
        Optional<OwnerRepo> result =
                new NodeIdResolver(cache).resolve(provider, new MutationNodeIdRef("R_1", null), "token");
    ...
    void cacheMiss_resolvesIssueShapeResponse_nestedUnderRepository() {
        ...
        Optional<OwnerRepo> result =
                new NodeIdResolver(cache).resolve(provider, new MutationNodeIdRef("I_1", null), "token");
    ...
    void seed_populatesCacheWithoutAnUpstreamCall() {
        ...
        new NodeIdResolver(cache).seed(provider, "R_1", new OwnerRepo("acme", "widgets"));

        assertEquals(Optional.of(new OwnerRepo("acme", "widgets")), cache.lookup("github", "R_1"));
    }

    private static final class InMemoryNodeIdCache implements NodeIdCache {
