...
class PushStorePersistenceHookTest {
    ...
    void validationResultHook_withIssues_transitionsToRejected() {
        ...
        ctx.addIssue(PushStepKind.AUTHOR_EMAIL, "Email blocked", "noreply@ address is not allowed");
    ...
    void validationResultHook_withIssues_commandsAreRejected() {
        ...
        ValidationContext ctx = new ValidationContext();
        ctx.addIssue(PushStepKind.COMMIT_MESSAGE, "WIP commit", "message contains blocked term");
        hook.validationResultHook(ctx).onPreReceive(rp, List.of(cmd));
    ...
    void validationResultHook_withIssues_rejectedRecordHasBlockedMessage() {
        ...
        ValidationContext ctx = new ValidationContext();
        ctx.addIssue(PushStepKind.AUTHOR_EMAIL, "Email blocked", "some detail");
        hook.validationResultHook(ctx).onPreReceive(rp, List.of(cmd));
    ...
    void validationResultHook_multipleIssues_allRecorded() {
        ...
        ValidationContext ctx = new ValidationContext();
        ctx.addIssue(PushStepKind.AUTHOR_EMAIL, "Email blocked", "noreply@ address");
        ctx.addIssue(PushStepKind.COMMIT_MESSAGE, "WIP commit", "message contains WIP");
