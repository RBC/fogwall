...
class ScmApiGitLabGateFilterTest {
    ...
    private ScmApiGitLabGateFilter filter() {
        return new ScmApiGitLabGateFilter(
                provider, accessEvaluator, new GitLabProjectIdResolver(projectIdCache), repoPermissionService);
    }
    ...
    void get_readDeniedByAccessRule_returns403() throws Exception {
        when(accessEvaluator.evaluate("gitlab", ScmApiAccessRule.Operation.READ))
    ...
    void allowlistedMutation_deniedByAccessRule_returns403() throws Exception {
        when(accessEvaluator.evaluate("gitlab", ScmApiAccessRule.Operation.MUTATE))
                .thenReturn(new ScmApiAccessEvaluator.Result.NotAllowed());
        var context = new ScmApiRequestContext();
