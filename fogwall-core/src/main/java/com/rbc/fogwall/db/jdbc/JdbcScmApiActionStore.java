...
public class JdbcScmApiActionStore implements ScmApiActionStore {
    ...
    public void save(ScmApiActionRecord record) {
        jdbc.update("""
                INSERT INTO scm_api_action_records (id, timestamp, provider, push_user, resolved_user, repo_owner,
                    repo_name, mutation_field, node_id, node_type, status, reason, variables_json,
                    user_agent, client_type)
                VALUES (:id, :timestamp, :provider, :pushUser, :resolvedUser, :repoOwner, :repoName, :mutationField,
