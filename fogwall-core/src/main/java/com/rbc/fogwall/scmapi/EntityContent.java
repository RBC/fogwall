...
/**
 * The caller-supplied prose carried by an SCM API entity mutation — a pull/merge request or issue title and
 ...
 */
public record EntityContent(String field, String text) {
     ...
     */
    public static List<EntityContent> fromGraphQlBody(JsonNode requestBody) {
    ...
    public static List<EntityContent> fromGitLabBody(JsonNode body) {
        return from(body, "title", "description", "body");
    }

    /** Forgejo: {@code body} for both descriptions and comments. */
    public static List<EntityContent> fromForgejoBody(JsonNode body) {
        return from(body, "title", "body");
    }

    private static List<EntityContent> from(JsonNode node, String... fields) {
        if (node == null || !node.isObject()) {
            return List.of();
        }
        List<EntityContent> content = new ArrayList<>();
        for (String field : fields) {
            JsonNode value = node.get(field);
            if (value != null && value.isString() && !value.asString().isBlank()) {
                content.add(new EntityContent(field, value.asString()));
            }
        }
        return content;
    }
