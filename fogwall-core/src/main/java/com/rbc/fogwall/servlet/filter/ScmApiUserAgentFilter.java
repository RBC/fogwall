...
public class ScmApiUserAgentFilter implements Filter {
    ...
    public void doFilter(ServletRequest request, ServletResponse response, FilterChain chain)
        ...
        if (requireKnownCli && !clientType.isKnownCli()) {
            String reason = "Client type " + clientType + " is not permitted by scm-api.require-known-cli";
            ...
            httpResponse.getOutputStream().write(("{\"error\":\"" + reason + "\"}").getBytes(StandardCharsets.UTF_8));
            return;
        }
