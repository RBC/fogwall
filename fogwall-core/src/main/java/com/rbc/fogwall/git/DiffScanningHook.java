...
public final class DiffScanningHook implements MandatoryFogwallHook {
        ...
        if (aggregateDiff == null || aggregateDiff.isBlank()) {
            ...
            if (!violations.isEmpty()) {
                for (Violation v : violations) {
                    validationContext.addIssue(PushStepKind.DIFF_SCAN, v.reason(), v.formattedDetail());
        ...
        for (ReceiveCommand cmd : commands) {
            ...
            try {
                CommitInspectionService.forEachIntroducedCommit(repo, baseCommit, tipCommit, (commit, diffs) -> {
                    ...
                    try {
                        String perCommitDiff = CommitInspectionService.formatDiffEntries(repo, diffs);
                        ...
                        if (violations.isEmpty()) {
                            ...
                            for (Violation v : violations) {
                                ...
                                validationContext.addIssue(PushStepKind.DIFF_SCAN, reason, detail);
