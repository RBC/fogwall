...
public interface SshScmLoginResolver {
    ...
    /**
     * Consults each resolver in order and returns the first login any of them proves, skipping nulls.
     *
     * <p>Order is the point rather than an implementation detail: whichever resolver runs first decides the cases both
     * can answer, and the rest only see what it left unresolved.
     */
    static SshScmLoginResolver firstOf(SshScmLoginResolver... resolvers) {
        List<SshScmLoginResolver> present =
                Arrays.stream(resolvers).filter(Objects::nonNull).toList();
        return (user, provider, connectingFingerprint) -> present.stream()
                .map(resolver -> resolver.resolveScmLogin(user, provider, connectingFingerprint))
