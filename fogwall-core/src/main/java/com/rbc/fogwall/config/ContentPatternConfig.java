 ...
 * content, commit messages, and the prose an SCM API entity carries. Operator-authored custom patterns are out of scope
 * here - that's already covered by {@code DiffScanConfig}'s {@code block.literals}/{@code block.patterns}.
 *
 ...
 *
 * <p><b>On the SCM API path, blocking.</b> That WARN rests on a reviewer downstream who sees it, and this path has none
 * - the request is forwarded or refused, with no held state to annotate. A warning recorded against text already
 * published upstream is not a control, so a match there refuses the request, the same as a blocked term or a detected
 * secret.
 *
 ...
 *
 * <p>{@code scanDiff}/{@code scanCommitMessages}/{@code scanScmApi} independently gate the three content sources - an
 * operator who considers commit messages low-risk (or wants to reduce push-summary noise) can disable that half without
 * affecting diff scanning, and vice versa. All default {@code true}, so enabling a bundle covers every surface it can
 * appear on.
 */
@Data
@Builder
