...
-- Provider-level access rules: is a provider allowed to serve READ/MUTATE/BOTH SCM API traffic at all. Deliberately
-- not the same model as access_rules (which matches a repo path in the URL) — GitHub's GraphQL requests don't carry
-- owner/repo in the URL, so this is coarser: provider-only, no path.
