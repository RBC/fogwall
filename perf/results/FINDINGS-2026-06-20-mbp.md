...
- **Clone and fetch** are comparable across both proxies in all configurations
  (HTTP, TLS, sequential, concurrent). Proxy overhead is negligible — Gitea is
  the bottleneck.

- **Push** is comparable sequentially but diverges under concurrency. The gap
  appears in both HTTP and TLS configurations, confirming it's architectural
  (clone caching strategy) rather than TLS-related.
...
- Modified `proxy/routes/index.ts` and `proxy/processors/pre-processor/parseAction.ts`
  to support plain HTTP as a proxy upstream target (reverted for TLS tests).
